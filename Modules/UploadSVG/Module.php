...
class Module extends ModuleBase implements ManagableModuleInterface {
	...
	public function handle_svg_file( $file ) {
		...
		if ( ! $this->user_can_upload_svg() ) {
			$file['error'] = __( 'Sorry, you are not allowed to upload SVG files.', 'bbe' );

			return $file;
		}

		if ( ! $this->sanitize( $file['tmp_name'] ) ) {
			error_log( 'SVG sanitization failed for: ' . $file_name );
			$file['error'] = __( "Sorry, this file couldn't be sanitized so for security reasons wasn't uploaded", 'bbe' );
		}
	...
	protected function sanitize( $file ) {
		...
		if ( apply_filters( 'dt-cr-svg_large_svg', false ) ) {
	...
	protected function svg_dimensions( $attachment_id ) {
		...
		$svg_dimensions = apply_filters( 'dt-cr-svg_dimensions', false, $attachment_id );
		...
		if ( $svg_file && ! empty( $metadata['width'] ) && ! empty( $metadata['height'] ) ) {
			...
			$use_attr = (bool) apply_filters( 'dt-cr-svg_use_width_height_attributes', false, $svg_file );
		...
		$this->svg_cache[ $attachment_id ] = apply_filters( 'dt-cr-svg_dimensions', $dimensions, $svg_file );
	...
	public function user_can_upload_svg() {
		...
		return (bool) apply_filters( 'dt-cr-svg_user_can_upload', $can_upload );
	}

	public static function get_title() {
		return __( 'Inline SVG', 'bbe' );
	}

	public static function get_label() {
		return __( 'Allow to upload and display the SVG icon', 'bbe' );
	}
}
