/** 
 * copy of @wordpress/block-editor/src/components/global-styles/shadow-panel-components.js
 */
/**
...
export function ShadowPopoverContainer( { shadow, onShadowChange, settings } ) {
	...
	return (
		<div className="block-editor-global-styles__shadow-popover-container">
			<VStack spacing={ 4 }>
				<Heading level={ 5 }>{ __( 'Drop shadow', 'bbe' ) }</Heading>
				<ShadowPresets
					presets={ shadows }
					activeShadow={ shadow }
...
export function ShadowPresets( { presets, activeShadow, onSelect } ) {
	return ! presets ? null : (
		<Composite
			...
			aria-label={ __( 'Drop shadows', 'bbe' ) }
		>
			...
			<Button __next40pxDefaultSize { ...toggleProps }>
				<HStack justify="flex-start">
					<Icon
						...
						size={ 24 }
					/>
					<FlexItem>{ __( 'Drop shadow', 'bbe' ) }</FlexItem>
				</HStack>
			</Button>
		);
	};
